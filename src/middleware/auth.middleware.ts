...
function authenticateToken(req: CustomAuthRequest, res: Response, next: NextFunction) {
  ...
  try {
    ...
    logger.warn({ ip: req.ip, err: errMessage }, "auth.token_verification_failed");
    return res.status(403).json({ error: "Invalid or expired token" });
  }
}

function requireAdmin(req: CustomAuthRequest, res: Response, next: NextFunction) {
...
function requireStudent(req: CustomAuthRequest, res: Response, next: NextFunction) {
