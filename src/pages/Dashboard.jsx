...
export default function Dashboard() {
  ...
  const displayEmail = userData?.email || 'ctansit@st.futminna.edu.ng';
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        setUserData(profile);
        setWalletBalance(parseFloat(profile?.wallet?.balance) || 0);

        // Fetch recent transfers/trips
        try {
          ...
          setRecentTrips(transfersResponse.data.data?.slice(0, 3) || []);
        } catch {
          setRecentTrips([]); // Don't crash dashboard if history fails
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const fetchRegisteredUsers = async () => {
      try {
        ...
        setRegisteredUsers(json.data.count);
      } catch (error) {
      ...
      <main className={styles.dashboardShell}>
        <header className={styles.topHeader}>
          <div className={styles.brandWrap}>
            <div className={styles.brandBadge}>CT</div>
          </div>
